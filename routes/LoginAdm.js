    ...
    create: async (req, res) => {
        try {
            ...
            if (SalvarDados.email != undefined && SalvarDados.senha != undefined) {
                Admin.find({ email: SalvarDados.email })
                    .then((resultado) => {
                        ...
                        if (resultado_tratado.senha === SalvarDados.senha) {
                            ...
                            Admin.updateOne({ email: SalvarDados.email }, { $set: { token: tokenGerado } })
                                .then(() => {
...
router.route('/loginAdmin').post((req, res) => loginAdm.create(req, res));
